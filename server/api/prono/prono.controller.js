...
function calculateScore() {
  ...
  Prono.findOne({
    ...
    if (errpronosEuro !== null) { // si pas de prono de référence, gestion de l'exception
      ...
      var QualifiedTeams = {
        'Round of 16': allTeamsRoundOf16,
        'Quarter Finals': allTeamsQuarter,
        'Semi Final': allTeamsSemi,
        'Final': allTeamsFinal
      };
      // console.log('QualifiedTeams', QualifiedTeams);
      var euroWinner = _.filter(pronosEuro.matchs, {
        '_id': '5724ec9a93d5d62c81c95b9c'
      })[0].winner;
      ...
      Prono.find({}).populate('user_id').lean().exec((err, pronos) => {
        _.each(pronos, (thisProno) => {
          ...
          var allTeamsQualif = [];
          var teamWinner = _.filter(thisProno.matchs, {
            '_id': '5724ec9a93d5d62c81c95b9c'
          })[0].winner;
          ...
          _.each(pronosEuro.matchs, (euroMatch, index) => {
            ...
            if (euroMatch.result !== null && euroMatch.result !== undefined) {
              pronoMatch = _.filter(thisProno.matchs, {
                '_id': euroMatch._id
              });
              ...
              if (pronoMatch.group.length > 1) {
                ...
                if (QualifiedTeams[pronoMatch.group].indexOf(pronoMatch.team1) > -1) {
                  ...
                  allTeamsQualif.push({
                    team: pronoMatch.team1,
                    points: 2,
                    phase: pronoMatch.group
                  });
                } else {
                  allTeamsQualif.push({
                    team: pronoMatch.team1,
